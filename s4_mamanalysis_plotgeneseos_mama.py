...
def load_and_analyze_data():
    ...
    # ----------------------------------------------------------------------
    # GENES DE INTERESSE
    GENES_OF_INTEREST = ['CLC', 'EPX', 'IL5RA', 'PRG2']
    ...
    colors = plt.cm.get_cmap('Set1', len(GENES_OF_INTEREST))
        ...
        for bar, absolute_count_val, percentage_val in zip(bars, absolute_count, heights):
            ...
            if bar.get_height() > 3: 
                ax.text(
                    ...
                    ha='center',
                    va='center',
                    color='black' if colors(i)[0] > 0.5 and colors(i)[1] > 0.5 and colors(i)[2] > 0.5 else 'white',
    ...
    print(f"O total da barra é a soma das frequências individuais, indicando a média de genes expressos por paciente no subtipo.")
