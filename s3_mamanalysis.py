...
# --- Constantes ---
# NOVO GRUPO DE GENES: CLC, EPX, IL5RA, PRG2
GENES_INTERESSE = ['CLC', 'EPX', 'IL5RA', 'PRG2']
...
def definir_grupos_comparacao(df):
    ...
    Adaptado para CLC, EPX, IL5RA, PRG2.
    """
    ...
    # 2. Definir o grupo complexo (Todos os Genes Expressos Simultaneamente)
    g_4_all = (
        df['CLC_expresso'] & 
        df['EPX_expresso'] & 
        df['IL5RA_expresso'] & 
    ...
    df['grupo_4_genes_vs_outros'] = np.where(
        g_4_all, 
        '_'.join(GENES_INTERESSE), # Todos expressos: CLC_EPX_IL5RA_PRG2
...
def main():
    ...
    """
    print("--- Iniciando Script de Geração de Imagens (CLC, EPX, IL5RA, PRG2) ---")
    ...
    grupos_para_plotar = [
        # O nome do grupo combinado foi atualizado no título e no nome curto
        ('grupo_4_genes_vs_outros', 'CLC_EPX_IL5RA_PRG2 vs. Demais', '4_genes'),
        # Os nomes dos grupos individuais foram atualizados
        ('grupo_CLC_vs_Nao', 'Expressa CLC vs. Não Expressa CLC', 'CLC'),
        ('grupo_EPX_vs_Nao', 'Expressa EPX vs. Não Expressa EPX', 'EPX'),
        ('grupo_IL5RA_vs_Nao', 'Expressa IL5RA vs. Não Expressa IL5RA', 'IL5RA'),
        ('grupo_PRG2_vs_Nao', 'Expressa PRG2 vs. Não Expressa PRG2', 'PRG2'),
    ]
...
if __name__ == "__main__":
    ...
    try:
        # Tentativa de usar uma fonte mais acadêmica, se disponível
        plt.rcParams['font.family'] = 'Times New Roman'
    except:
        print("Fonte Times New Roman não encontrada, usando fonte padrão.")
